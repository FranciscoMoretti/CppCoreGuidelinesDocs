# <a href="#" name="S-interfaces"></a>I: Interfaces
...
* [(I.1: Make interfaces explicit)](/interfaces#Ri-explicit)
* [(I.2: Avoid non-`const` global variables)](/interfaces#Ri-global)
* [(I.3: Avoid singletons)](/interfaces#Ri-singleton)
* [(I.4: Make interfaces precisely and strongly typed)](/interfaces#Ri-typed)
* [(I.5: State preconditions (if any))](/interfaces#Ri-pre)
* [(I.6: Prefer `Expects()` for expressing preconditions)](/interfaces#Ri-expects)
* [(I.7: State postconditions)](/interfaces#Ri-post)
* [(I.8: Prefer `Ensures()` for expressing postconditions)](/interfaces#Ri-ensures)
* [(I.9: If an interface is a template, document its parameters using concepts)](/interfaces#Ri-concepts)
* [(I.10: Use exceptions to signal a failure to perform a required task)](/interfaces#Ri-except)
* [(I.11: Never transfer ownership by a raw pointer (`T*`) or reference (`T&`))](/interfaces#Ri-raw)
* [(I.12: Declare a pointer that must not be null as `not_null`)](/interfaces#Ri-nullptr)
* [(I.13: Do not pass an array as a single pointer)](/interfaces#Ri-array)
* [(I.22: Avoid complex initialization of global objects)](/interfaces#Ri-global-init)
* [(I.23: Keep the number of function arguments low)](/interfaces#Ri-nargs)
* [(I.24: Avoid adjacent parameters that can be invoked by the same arguments in either order with different meaning)](/interfaces#Ri-unrelated)
* [(I.25: Prefer empty abstract classes as interfaces to class hierarchies)](/interfaces#Ri-abstract)
* [(I.26: If you want a cross-compiler ABI, use a C-style subset)](/interfaces#Ri-abi)
* [(I.27: For stable library ABI, consider the Pimpl idiom)](/interfaces#Ri-pimpl)
* [(I.30: Encapsulate rule violations)](/interfaces#Ri-encapsulate)

**See also**:

* [(F: Functions)](/functions#S-functions)
* [(C.concrete: Concrete types)](/class#SS-concrete)
* [(C.hier: Class hierarchies)](/class#SS-hier)
...
### <a href="#" name="Ri-explicit"></a>I.1: Make interfaces explicit
...
### <a href="#" name="Ri-global"></a>I.2: Avoid non-`const` global variables
...
**References**: See the [(rules for calling functions)](/functions#SS-call).
...
### <a href="#" name="Ri-singleton"></a>I.3: Avoid singletons
...
### <a href="#" name="Ri-typed"></a>I.4: Make interfaces precisely and strongly typed
...
### <a href="#" name="Ri-pre"></a>I.5: State preconditions (if any)
...
**References**: `Expects()` is described in [(GSL)](/gsl#S-gsl).
...
### <a href="#" name="Ri-expects"></a>I.6: Prefer `Expects()` for expressing preconditions
...
No, using `unsigned` is not a good way to sidestep the problem of [(ensuring that a value is non-negative)](/expr#Res-nonnegative).
...
### <a href="#" name="Ri-post"></a>I.7: State postconditions
...
Better still, use [(RAII)](/resource#Rr-raii) to ensure that the postcondition ("the lock must be released") is enforced in code:
...
### <a href="#" name="Ri-ensures"></a>I.8: Prefer `Ensures()` for expressing postconditions
...
**Alternative**: Postconditions of the form "this resource must be released" are best expressed by [(RAII)](/resource#Rr-raii).
...
### <a href="#" name="Ri-concepts"></a>I.9: If an interface is a template, document its parameters using concepts
...
**See also**: [((Generic programming)](/templates#SS-GP) and [concepts)](/templates#SS-concepts).
...
### <a href="#" name="Ri-except"></a>I.10: Use exceptions to signal a failure to perform a required task
...
* A good rule for performance critical code is to move checking outside the [(critical)](/performance#Rper-critical) part of the code.
* In the longer term, more regular code gets better optimized.
* Always carefully [(measure)](/performance#Rper-measure) before making performance claims.

**See also**: [((I.5)](/interfaces#Ri-pre) and [I.7)](/interfaces#Ri-post) for reporting precondition and postcondition violations.
...
### <a href="#" name="Ri-raw"></a>I.11: Never transfer ownership by a raw pointer (`T*`) or reference (`T&`)
...
**Alternative**: [(Pass ownership)](/resource#Rr-smartptrparam) using a "smart pointer", such as `unique_ptr` (for exclusive ownership) and `shared_ptr` (for shared ownership).
...
In that case, mark owning pointers using `owner` from the [(guidelines support library)](/gsl#S-gsl):
...
**See also**: [(((Argument passing)](/functions#Rf-conventional), [use of smart pointer arguments)](/resource#Rr-smartptrparam), and [value return)](/functions#Rf-value-return).
...
### <a href="#" name="Ri-nullptr"></a>I.12: Declare a pointer that must not be null as `not_null`
...
`not_null` is defined in the [(guidelines support library)](/gsl#S-gsl).
...
### <a href="#" name="Ri-array"></a>I.13: Do not pass an array as a single pointer
...
But when doing so, use `std::string_view` or `span<char>` from the [(GSL)](/gsl#S-gsl) to prevent range errors.
...
### <a href="#" name="Ri-global-init"></a>I.22: Avoid complex initialization of global objects
...
### <a href="#" name="Ri-nargs"></a>I.23: Keep the number of function arguments low
...
### <a href="#" name="Ri-unrelated"></a>I.24: Avoid adjacent parameters that can be invoked by the same arguments in either order with different meaning
...
### <a href="#" name="Ri-abstract"></a>I.25: Prefer empty abstract classes as interfaces to class hierarchies
...
### <a href="#" name="Ri-abi"></a>I.26: If you want a cross-compiler ABI, use a C-style subset
...
### <a href="#" name="Ri-pimpl"></a>I.27: For stable library ABI, consider the Pimpl idiom
...
### <a href="#" name="Ri-encapsulate"></a>I.30: Encapsulate rule violations
...
This violated the rule [(against uninitialized variables)](/expr#Res-always),
the rule against [(ignoring ownership)](/interfaces#Ri-raw),
and the rule [(against magic constants)](/expr#Res-magic).
