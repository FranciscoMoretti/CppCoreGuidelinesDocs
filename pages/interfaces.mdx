...
**References**: `Expects()` is described in [GSL](/stdlib#S-gsl).
...
In that case, mark owning pointers using `owner` from the [guidelines support library](/stdlib#S-gsl):
...
`not_null` is defined in the [guidelines support library](/stdlib#S-gsl).
...
But when doing so, use `std::string_view` or `span<char>` from the [GSL](/stdlib#S-gsl) to prevent range errors.
