# <a href="#" name="S-concurrency"></a>CP: Concurrency and parallelism
...
* [(CP.1: Assume that your code will run as part of a multi-threaded program)](/concurrency#Rconc-multi)
* [(CP.2: Avoid data races)](/concurrency#Rconc-races)
* [(CP.3: Minimize explicit sharing of writable data)](/concurrency#Rconc-data)
* [(CP.4: Think in terms of tasks, rather than threads)](/concurrency#Rconc-task)
* [(CP.8: Don't try to use `volatile` for synchronization)](/concurrency#Rconc-volatile)
* [(CP.9: Whenever feasible use tools to validate your concurrent code)](/concurrency#Rconc-tools)

**See also**:

* [(CP.con: Concurrency)](/concurrency#SScp-con)
* [(CP.coro: Coroutines)](/concurrency#SScp-coro)
* [(CP.par: Parallelism)](/concurrency#SScp-par)
* [(CP.mess: Message passing)](/concurrency#SScp-mess)
* [(CP.vec: Vectorization)](/concurrency#SScp-vec)
* [(CP.free: Lock-free programming)](/concurrency#SScp-free)
* [(CP.etc: Etc. concurrency rules)](/concurrency#SScp-etc)

### <a href="#" name="Rconc-multi"></a>CP.1: Assume that your code will run as part of a multi-threaded program
...
### <a href="#" name="Rconc-races"></a>CP.2: Avoid data races
...
For further information of how to use synchronization well to eliminate data races, please consult a good book about concurrency (See [(Carefully study the literature)](/concurrency#Rconc-literature)).
...
### <a href="#" name="Rconc-data"></a>CP.3: Minimize explicit sharing of writable data
...
See also [((CP.mess: Message Passing)](/concurrency#SScp-mess) and [CP.31: prefer pass by value)](/concurrency#Rconc-data-by-value).

##### Enforcement

???

### <a href="#" name="Rconc-task"></a>CP.4: Think in terms of tasks, rather than threads
...
### <a href="#" name="Rconc-volatile"></a>CP.8: Don't try to use `volatile` for synchronization
...
[((rare) proper uses of `volatile`)](/concurrency#Rconc-volatile2)

### <a href="#" name="Rconc-tools"></a>CP.9: Whenever feasible use tools to validate your concurrent code
...
## <a href="#" name="SScp-con"></a>CP.con: Concurrency

This section focuses on relatively ad-hoc uses of multiple threads communicating through shared data.

* For parallel algorithms, see [(parallelism)](/concurrency#SScp-par)
* For inter-task communication without explicit sharing, see [(messaging)](/concurrency#SScp-mess)
* For vector parallel code, see [(vectorization)](/concurrency#SScp-vec)
* For lock-free programming, see [(lock free)](/concurrency#SScp-free)

Concurrency rule summary:

* [(CP.20: Use RAII, never plain `lock()`/`unlock()`)](/concurrency#Rconc-raii)
* [(CP.21: Use `std::lock()` or `std::scoped_lock` to acquire multiple `mutex`es)](/concurrency#Rconc-lock)
* [(CP.22: Never call unknown code while holding a lock (e.g., a callback))](/concurrency#Rconc-unknown)
* [(CP.23: Think of a joining `thread` as a scoped container)](/concurrency#Rconc-join)
* [(CP.24: Think of a `thread` as a global container)](/concurrency#Rconc-detach)
* [CP.25: Prefer `gsl::joining_thread` over `std::thread`](#Rconc-joining_thread)
* [CP.26: Don't `detach()` a thread](#Rconc-detached_thread)
* [(CP.31: Pass small amounts of data between threads by value, rather than by reference or pointer)](/concurrency#Rconc-data-by-value)
* [(CP.32: To share ownership between unrelated `thread`s use `shared_ptr`)](/concurrency#Rconc-shared)
* [(CP.40: Minimize context switching)](/concurrency#Rconc-switch)
* [(CP.41: Minimize thread creation and destruction)](/concurrency#Rconc-create)
* [(CP.42: Don't `wait` without a condition)](/concurrency#Rconc-wait)
* [(CP.43: Minimize time spent in a critical section)](/concurrency#Rconc-time)
* [(CP.44: Remember to name your `lock_guard`s and `unique_lock`s)](/concurrency#Rconc-name)
* [(CP.50: Define a `mutex` together with the data it guards. Use `synchronized_value<T>` where possible)](/concurrency#Rconc-mutex)
...
### <a href="#" name="Rconc-raii"></a>CP.20: Use RAII, never plain `lock()`/`unlock()`
...
### <a href="#" name="Rconc-lock"></a>CP.21: Use `std::lock()` or `std::scoped_lock` to acquire multiple `mutex`es
...
### <a href="#" name="Rconc-unknown"></a>CP.22: Never call unknown code while holding a lock (e.g., a callback)
...
### <a href="#" name="Rconc-join"></a>CP.23: Think of a joining `thread` as a scoped container
...
### <a href="#" name="Rconc-detach"></a>CP.24: Think of a `thread` as a global container
...
### <a href="#" name="Rconc-joining_thread"></a>CP.25: Prefer `gsl::joining_thread` over `std::thread`
...
### <a href="#" name="Rconc-detached_thread"></a>CP.26: Don't `detach()` a thread
...
### <a href="#" name="Rconc-data-by-value"></a>CP.31: Pass small amounts of data between threads by value, rather than by reference or pointer
...
### <a href="#" name="Rconc-shared"></a>CP.32: To share ownership between unrelated `thread`s use `shared_ptr`
...
### <a href="#" name="Rconc-switch"></a>CP.40: Minimize context switching
...
### <a href="#" name="Rconc-create"></a>CP.41: Minimize thread creation and destruction
...
### <a href="#" name="Rconc-wait"></a>CP.42: Don't `wait` without a condition
...
### <a href="#" name="Rconc-time"></a>CP.43: Minimize time spent in a critical section
...
But that compromises safety and violates the [(use RAII)](/concurrency#Rconc-raii) rule.
...
### <a href="#" name="Rconc-name"></a>CP.44: Remember to name your `lock_guard`s and `unique_lock`s
...
### <a href="#" name="Rconc-mutex"></a>CP.50: Define a `mutex` together with the data it guards. Use `synchronized_value<T>` where possible
...
## <a href="#" name="SScp-coro"></a>CP.coro: Coroutines

This section focuses on uses of coroutines.

Coroutine rule summary:

* [(CP.51: Do not use capturing lambdas that are coroutines)](/concurrency#Rcoro-capture)
* [(CP.52: Do not hold locks or other synchronization primitives across suspension points)](/concurrency#Rcoro-locks)
* [(CP.53: Parameters to coroutines should not be passed by reference)](/concurrency#Rcoro-reference-parameters)

### <a href="#" name="Rcoro-capture"></a>CP.51: Do not use capturing lambdas that are coroutines
...
### <a href="#" name="Rcoro-locks"></a>CP.52: Do not hold locks or other synchronization primitives across suspension points
...
This pattern is also bad for performance. When a suspension point is reached, such as co\_await, execution of the current function stops and other code begins to run. It may be a long period of time before the coroutine resumes. For that entire duration the lock will be held and cannot be acquired by other threads to perform work.

##### Enforcement

Flag all lock guards that are not destructed before a coroutine suspends.

### <a href="#" name="Rcoro-reference-parameters"></a>CP.53: Parameters to coroutines should not be passed by reference

##### Reason

Once a coroutine reaches the first suspension point, such as a co\_await, the synchronous portion returns. After that point any parameters passed by reference are dangling. Any usage beyond that is undefined behavior which may include writing to freed memory.
...
The same danger applies to output parameters.  [(F.20: For "out" output values, prefer return values to output parameters)](/functions#Rf-out) discourages output parameters.  Coroutines should avoid them entirely.

##### Enforcement

Flag all reference parameters to a coroutine.

## <a href="#" name="SScp-par"></a>CP.par: Parallelism
...
## <a href="#" name="SScp-mess"></a>CP.mess: Message passing
...
* [(CP.60: Use a `future` to return a value from a concurrent task)](/concurrency#Rconc-future)
* [(CP.61: Use `async()` to spawn concurrent tasks)](/concurrency#Rconc-async)
* message queues
* messaging libraries
...
### <a href="#" name="Rconc-future"></a>CP.60: Use a `future` to return a value from a concurrent task
...
### <a href="#" name="Rconc-async"></a>CP.61: Use `async()` to spawn concurrent tasks

##### Reason

Similar to [(R.12)](/resource#Rr-immediate-alloc), which tells you to avoid raw owning pointers, you should
...
## <a href="#" name="SScp-vec"></a>CP.vec: Vectorization
...
## <a href="#" name="SScp-free"></a>CP.free: Lock-free programming
...
* [(CP.100: Don't use lock-free programming unless you absolutely have to)](/concurrency#Rconc-lockfree)
* [(CP.101: Distrust your hardware/compiler combination)](/concurrency#Rconc-distrust)
* [(CP.102: Carefully study the literature)](/concurrency#Rconc-literature)
* how/when to use atomics
* avoid starvation
* use a lock-free data structure rather than hand-crafting specific lock-free access
* [(CP.110: Do not write your own double-checked locking for initialization)](/concurrency#Rconc-double)
* [(CP.111: Use a conventional pattern if you really need double-checked locking)](/concurrency#Rconc-double-pattern)
* how/when to compare and swap

### <a href="#" name="Rconc-lockfree"></a>CP.100: Don't use lock-free programming unless you absolutely have to
...
[Atomic variables](#???) can be used simply and safely, as long as you are using the sequentially consistent memory model (memory\_order\_seq\_cst), which is the default.
...
### <a href="#" name="Rconc-distrust"></a>CP.101: Distrust your hardware/compiler combination
...
### <a href="#" name="Rconc-literature"></a>CP.102: Carefully study the literature
...
### <a href="#" name="Rconc-double"></a>CP.110: Do not write your own double-checked locking for initialization

##### Reason

Since C++11, static local variables are now initialized in a thread-safe way. When combined with the RAII pattern, static local variables can replace the need for writing your own double-checked locking for initialization. std::call\_once can also achieve the same purpose. Use either static local variables of C++11 or std::call\_once instead of writing your own double-checked locking for initialization.

##### Example

Example with std::call\_once.
...
### <a href="#" name="Rconc-double-pattern"></a>CP.111: Use a conventional pattern if you really need double-checked locking

##### Reason

Double-checked locking is easy to mess up. If you really need to write your own double-checked locking, in spite of the rules [((CP.110: Do not write your own double-checked locking for initialization)](/concurrency#Rconc-double) and [CP.100: Don't use lock-free programming unless you absolutely have to)](/concurrency#Rconc-lockfree), then do it in a conventional pattern.

The uses of the double-checked locking pattern that are not in violation of [(CP.110: Do not write your own double-checked locking for initialization)](/concurrency#Rconc-double) arise when a non-thread-safe action is both hard and rare, and there exists a fast thread-safe test that can be used to guarantee that the action is not needed, but cannot be used to guarantee the converse.

##### Example, bad

The use of volatile does not make the first check thread-safe, see also [(CP.200: Use `volatile` only to talk to non-C++ memory)](/concurrency#Rconc-volatile2)
...
## <a href="#" name="SScp-etc"></a>CP.etc: Etc. concurrency rules

These rules defy simple categorization:

* [(CP.200: Use `volatile` only to talk to non-C++ memory)](/concurrency#Rconc-volatile2)
* [(CP.201: ??? Signals)](/concurrency#Rconc-signal)

### <a href="#" name="Rconc-volatile2"></a>CP.200: Use `volatile` only to talk to non-C++ memory
...
In C++, unlike in some other languages, `volatile` has [(nothing to do with synchronization)](/concurrency#Rconc-volatile).
...
### <a href="#" name="Rconc-signal"></a>CP.201: ??? Signals
