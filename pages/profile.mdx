# <a href="#" name="S-profile"></a>Pro: Profiles
...
We must aim for [gradual adoption](/modernizing#S-modernizing).
...
* [Pro.type: Type safety](/profile#SS-type)

* [Pro.bounds: Bounds safety](/profile#SS-bounds)

* [Pro.lifetime: Lifetime safety](/profile#SS-lifetime)
...
## <a href="#" name="SS-type"></a>Pro.safety: Type-safety profile
...
Note that the safety is intended to be complete when combined also with [Bounds safety](/profile#SS-bounds) and [Lifetime safety](/profile#SS-lifetime).
...
* <a href="#" name="Pro-type-avoidcasts"></a>Type.1: [Avoid casts](/expr#Res-casts):


  1. <a href="#" name="Pro-type-reinterpretcast"></a>Don't use `reinterpret_cast`; A strict version of [Avoid casts](/expr#Res-casts) and [prefer named casts](/expr#Res-casts-named).

  2. <a href="#" name="Pro-type-arithmeticcast"></a>Don't use `static_cast` for arithmetic types; A strict version of [Avoid casts](/expr#Res-casts) and [prefer named casts](/expr#Res-casts-named).

  3. <a href="#" name="Pro-type-identitycast"></a>Don't cast between pointer types where the source type and the target type are the same; A strict version of [Avoid casts](/expr#Res-casts).

  4. <a href="#" name="Pro-type-implicitpointercast"></a>Don't cast between pointer types when the conversion could be implicit; A strict version of [Avoid casts](/expr#Res-casts).

* <a href="#" name="Pro-type-downcast"></a>Type.2: Don't use `static_cast` to downcast:
[Use `dynamic_cast` instead](#Rh-dynamic_cast).
* <a href="#" name="Pro-type-constcast"></a>Type.3: Don't use `const_cast` to cast away `const` (i.e., at all):
[Don't cast away const](/expr#Res-casts-const).

* <a href="#" name="Pro-type-cstylecast"></a>Type.4: Don't use C-style `(T)expression` or functional `T(expression)` casts:
Prefer [construction](/expr#Res-construct) or [named casts](/expr#Res-casts-named) or `T{expression}`.



* <a href="#" name="Pro-type-init"></a>Type.5: Don't use a variable before it has been initialized:
[always initialize](/expr#Res-always).

* <a href="#" name="Pro-type-memberinit"></a>Type.6: Always initialize a member variable:
[always initialize](/expr#Res-always),
...
* <a href="#" name="Pro-type-unon"></a>Type.7: Avoid naked union:
[Use `variant` instead](/class#Ru-naked).

* <a href="#" name="Pro-type-varargs"></a>Type.8: Avoid varargs:
...
Note that this type-safety can be complete only if we also have [Bounds safety](/profile#SS-bounds) and [Lifetime safety](/profile#SS-lifetime).
...
## <a href="#" name="SS-bounds"></a>Pro.bounds: Bounds safety profile
...
Bounds safety is intended to be complete only when combined with [Type safety](/profile#SS-type) and [Lifetime safety](/profile#SS-lifetime),
...
* <a href="#" name="Pro-bounds-arithmetic"></a>Bounds.1: Don't use pointer arithmetic. Use `span` instead:
[Pass pointers to single objects (only)](/interfaces#Ri-array) and [Keep pointer arithmetic simple](/expr#Res-ptr).



* <a href="#" name="Pro-bounds-arrayindex"></a>Bounds.2: Only index into arrays using constant expressions:
[Pass pointers to single objects (only)](/interfaces#Ri-array) and [Keep pointer arithmetic simple](/expr#Res-ptr).



* <a href="#" name="Pro-bounds-decay"></a>Bounds.3: No array-to-pointer decay:
[Pass pointers to single objects (only)](/interfaces#Ri-array) and [Keep pointer arithmetic simple](/expr#Res-ptr).



* <a href="#" name="Pro-bounds-stdlib"></a>Bounds.4: Don't use standard-library functions and types that are not bounds-checked:
...
## <a href="#" name="SS-lifetime"></a>Pro.lifetime: Lifetime safety profile
...
* <a href="#" name="Pro-lifetime-invalid-deref"></a>Lifetime.1: Don't dereference a possibly invalid pointer:
