# <a href="#" name="S-functions"></a>F: Functions
...
* [F.1: "Package" meaningful operations as carefully named functions](/functions#Rf-package)

* [F.2: A function should perform a single logical operation](/functions#Rf-logical)

* [F.3: Keep functions short and simple](/functions#Rf-single)

* [F.4: If a function might have to be evaluated at compile time, declare it `constexpr`](/functions#Rf-constexpr)

* [F.5: If a function is very small and time-critical, declare it inline](/functions#Rf-inline)

* [F.6: If your function must not throw, declare it `noexcept`](/functions#Rf-noexcept)

* [F.7: For general use, take `T*` or `T&` arguments rather than smart pointers](/functions#Rf-smart)

* [F.8: Prefer pure functions](/functions#Rf-pure)

* [F.9: Unused parameters should be unnamed](/functions#Rf-unused)


Parameter passing expression rules:

* [F.15: Prefer simple and conventional ways of passing information](/functions#Rf-conventional)

* [F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const`](/functions#Rf-in)

* [F.17: For "in-out" parameters, pass by reference to non-`const`](/functions#Rf-inout)

* [F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter](/functions#Rf-consume)

* [F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter](/functions#Rf-forward)

* [F.20: For "out" output values, prefer return values to output parameters](/functions#Rf-out)

* [F.21: To return multiple "out" values, prefer returning a struct or tuple](/functions#Rf-out-multi)

* [F.60: Prefer `T*` over `T&` when "no argument" is a valid option](/functions#Rf-ptr-ref)


Parameter passing semantic rules:

* [F.22: Use `T*` or `owner<T*>` to designate a single object](/functions#Rf-ptr)

* [F.23: Use a `not_null<T>` to indicate that "null" is not a valid value](/functions#Rf-nullptr)

* [F.24: Use a `span<T>` or a `span_p<T>` to designate a half-open sequence](/functions#Rf-range)

* [F.25: Use a `zstring` or a `not_null<zstring>` to designate a C-style string](/functions#Rf-zstring)
...
<a href="#" name="Rf-value-return"></a>Value return semantic rules:

* [F.42: Return a `T*` to indicate a position (only)](/functions#Rf-return-ptr)

* [F.43: Never (directly or indirectly) return a pointer or a reference to a local object](/functions#Rf-dangle)

* [F.44: Return a `T&` when copy is undesirable and "returning no object" isn't needed](/functions#Rf-return-ref)

* [F.45: Don't return a `T&&`](/functions#Rf-return-ref-ref)

* [F.46: `int` is the return type for `main()`](/functions#Rf-main)

* [F.47: Return `T&` from assignment operators](/functions#Rf-assignment-op)

* [F.48: Don't `return std::move(local)`](/functions#Rf-return-move-local)
...
* [F.51: Where there is a choice, prefer default arguments over overloading](/functions#Rf-default-args)
...
* [F.54: If you capture `this`, capture all variables explicitly (no default capture)](/functions#Rf-this-capture)

* [F.55: Don't use `va_arg` arguments](/functions#F-varargs)

* [F.56: Avoid unnecessary condition nesting](/functions#F-nesting)


Functions have strong similarities to lambdas and function objects.

**See also**: [C.lambdas: Function objects and lambdas](/class#SS-lambdas)


## <a href="#" name="SS-fct-def"></a>F.def: Function definitions

A function definition is a function declaration that also specifies the function's implementation, the function body.

### <a href="#" name="Rf-package"></a>F.1: "Package" meaningful operations as carefully named functions
...
The rule [Keep functions short and simple](/functions#Rf-single) implies "Keep loop bodies short."

Similarly, lambdas used as callback arguments are sometimes non-trivial, yet unlikely to be reusable.

##### Enforcement

* See [Keep functions short and simple](/functions#Rf-single)

* Flag identical and very similar lambdas used in different places.

### <a href="#" name="Rf-logical"></a>F.2: A function should perform a single logical operation
...
### <a href="#" name="Rf-single"></a>F.3: Keep functions short and simple
...
### <a href="#" name="Rf-constexpr"></a>F.4: If a function might have to be evaluated at compile time, declare it `constexpr`
...
### <a href="#" name="Rf-inline"></a>F.5: If a function is very small and time-critical, declare it `inline`
...
### <a href="#" name="Rf-noexcept"></a>F.6: If your function must not throw, declare it `noexcept`
...
See also [C.44](/class#Rc-default00).
...
### <a href="#" name="Rf-smart"></a>F.7: For general use, take `T*` or `T&` arguments rather than smart pointers
...
We can catch many common cases of dangling pointers statically (see [lifetime safety profile](/profile#SS-lifetime)). Function arguments naturally live for the lifetime of the function call, and so have fewer lifetime problems.
...
* [Prefer `T*` over `T&` when "no argument" is a valid option](/functions#Rf-ptr-ref)

* [Smart pointer rule summary](/resource#Rr-summary-smartptrs)


### <a href="#" name="Rf-pure"></a>F.8: Prefer pure functions
...
### <a href="#" name="Rf-unused"></a>F.9: Unused parameters should be unnamed
...
## <a href="#" name="SS-call"></a>F.call: Parameter passing

There are a variety of ways to pass parameters to a function and to return values.

### <a href="#" name="Rf-conventional"></a>F.15: Prefer simple and conventional ways of passing information
...
For passing sequences of characters see [String](/stdlib#SS-string).


### <a href="#" name="Rf-in"></a>F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const`
...
* If the function is going to unconditionally move from the argument, take it by `&&`. See [F.18](/functions#Rf-consume).

* If the function is going to keep a copy of the argument, in addition to passing by `const&` (for lvalues),
  add an overload that passes the parameter by `&&` (for rvalues) and in the body `std::move`s it to its destination. Essentially this overloads a "will-move-from"; see [F.18](/functions#Rf-consume).

* In special cases, such as multiple "input + copy" parameters, consider using perfect forwarding. See [F.19](/functions#Rf-forward).
...
* Passing arguments as `T&&` "for efficiency".
  Most rumors about performance advantages from passing by `&&` are false or brittle (but see [F.18](/functions#Rf-consume) and [F.19](/functions#Rf-forward)).



* Returning `const T&` from assignments and similar operations (see [F.47](/functions#Rf-assignment-op).)
...
### <a href="#" name="Rf-inout"></a>F.17: For "in-out" parameters, pass by reference to non-`const`
...
### <a href="#" name="Rf-consume"></a>F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter
...
[That could be dangerous](/class#Rc-move-semantic).
...
### <a href="#" name="Rf-forward"></a>F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter
...
### <a href="#" name="Rf-out"></a>F.20: For "out" output values, prefer return values to output parameters
...
If you have multiple values to return, [use a tuple](/functions#Rf-out-multi) or similar multi-member type.
...
* To reuse an object that carries capacity (e.g., `std::string`, `std::vector`) across multiple calls to the function in an inner loop: [treat it as an in/out parameter and pass by reference](/functions#Rf-out-multi).
...
### <a href="#" name="Rf-out-multi"></a>F.21: To return multiple "out" values, prefer returning a struct or tuple
...
In such cases, passing the object by reference [`T&`](/functions#Rf-inout) is usually the right technique.
...
### <a href="#" name="Rf-ptr-ref"></a>F.60: Prefer `T*` over `T&` when "no argument" is a valid option
...
### <a href="#" name="Rf-ptr"></a>F.22: Use `T*` or `owner<T*>` to designate a single object
...
**See also**: [Support library](/gsl#S-gsl)


**See also**: [Do not pass an array as a single pointer](/interfaces#Ri-array)
...
### <a href="#" name="Rf-nullptr"></a>F.23: Use a `not_null<T>` to indicate that "null" is not a valid value
...
### <a href="#" name="Rf-range"></a>F.24: Use a `span<T>` or a `span_p<T>` to designate a half-open sequence
...
### <a href="#" name="Rf-zstring"></a>F.25: Use a `zstring` or a `not_null<zstring>` to designate a C-style string
...
### <a href="#" name="Rf-unique_ptr"></a>F.26: Use a `unique_ptr<T>` to transfer ownership where a pointer is needed

##### Reason

Using `unique_ptr` is the cheapest way to pass a pointer safely.

**See also**: [C.50](/class#Rc-factory) regarding when to return a `shared_ptr` from a factory.
...
### <a href="#" name="Rf-shared_ptr"></a>F.27: Use a `shared_ptr<T>` to share ownership
...
### <a href="#" name="Rf-return-ptr"></a>F.42: Return a `T*` to indicate a position (only)
...
A reference is often a superior alternative to a pointer [if there is no need to use `nullptr`](/functions#Rf-ptr-ref) or [if the object referred to should not change](???).


##### Note

Do not return a pointer to something that is not in the caller's scope; see [F.43](/functions#Rf-dangle).
...
### <a href="#" name="Rf-dangle"></a>F.43: Never (directly or indirectly) return a pointer or a reference to a local object
...
### <a href="#" name="Rf-return-ref"></a>F.44: Return a `T&` when copy is undesirable and "returning no object" isn't needed
...
### <a href="#" name="Rf-return-ref-ref"></a>F.45: Don't return a `T&&`
...
The [lifetime safety profile](/profile#SS-lifetime) will (when completely implemented) catch such problems.
...
then, the temporary is guaranteed to outlive the function call (see [F.18](/functions#Rf-consume) and [F.19](/functions#Rf-forward)).
...
### <a href="#" name="Rf-main"></a>F.46: `int` is the return type for `main()`
...
### <a href="#" name="Rf-assignment-op"></a>F.47: Return `T&` from assignment operators
...
### <a href="#" name="Rf-return-move-local"></a>F.48: Don't `return std::move(local)`
...
### <a href="#" name="Rf-capture-vs-overload"></a>F.50: Use a lambda when a function won't do (to capture local variables, or to write a local function)
...
### <a href="#" name="Rf-default-args"></a>F.51: Where there is a choice, prefer default arguments over overloading
...
[Default arguments for virtual functions](/class#Rh-virtual-default-arg)
...
### <a href="#" name="Rf-reference-capture"></a>F.52: Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms
...
### <a href="#" name="Rf-value-capture"></a>F.53: Avoid capturing by reference in lambdas that will be used non-locally, including returned, stored on the heap, or passed to another thread
...
### <a href="#" name="Rf-this-capture"></a>F.54: If you capture `this`, capture all variables explicitly (no default capture)
...
### <a href="#" name="F-varargs"></a>F.55: Don't use `va_arg` arguments
...
### <a href="#" name="F-nesting"></a>F.56: Avoid unnecessary condition nesting
