...
For naming, commenting, and indentation rules, see [NL: Naming and layout](/stdlib#S-naming).
...
This rule is part of the [bounds-safety profile](/stdlib#SS-bounds).
...
The [type profile](/stdlib#Pro-type-reinterpretcast) bans `reinterpret_cast` and C-style casts.
...
* Flag [identity casts](/stdlib#Pro-type-identitycast) between pointer types, where the source and target types are the same (/stdlib#Pro-type-identitycast).

* Flag an explicit pointer cast that could be [implicit](/stdlib#Pro-type-implicitpointercast).
...
* The [type profile](/stdlib#Pro-type-reinterpretcast) bans `reinterpret_cast`.

* The [type profile](/stdlib#Pro-type-arithmeticcast) warns when using `static_cast` between arithmetic types.
...
* Flag `const_cast`s.
* This rule is part of the [type-safety profile](/stdlib#Pro-type-constcast) for the related Profile.
...
Never write `std::move()` on a const object, it is silently transformed into a copy (see Item 23 in [Meyers15](/stdlib#Meyers15))
...
This is a major part of the discussion of [C++'s model for type- and resource-safety](/stdlib#Stroustrup15).
...
* Use the [bounds profile](/stdlib#SS-bounds) to avoid range errors.
...
This rule is part of the [lifetime safety profile](/stdlib#SS-lifetime)
