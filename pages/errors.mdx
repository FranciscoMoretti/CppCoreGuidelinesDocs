# <a href="#" name="S-errors"></a>E: Error handling
...
* [(E.1: Develop an error-handling strategy early in a design)](/errors#Re-design)

* [(E.2: Throw an exception to signal that a function can't perform its assigned task)](/errors#Re-throw)

* [(E.3: Use exceptions for error handling only)](/errors#Re-errors)

* [(E.4: Design your error-handling strategy around invariants)](/errors#Re-design-invariants)

* [(E.5: Let a constructor establish an invariant, and throw if it cannot)](/errors#Re-invariant)

* [(E.6: Use RAII to prevent leaks)](/errors#Re-raii)

* [(E.7: State your preconditions)](/errors#Re-precondition)

* [(E.8: State your postconditions)](/errors#Re-postcondition)

* [(E.12: Use `noexcept` when exiting a function because of a `throw` is impossible or unacceptable)](/errors#Re-noexcept)

* [(E.13: Never throw while being the direct owner of an object)](/errors#Re-never-throw)

* [(E.14: Use purpose-designed user-defined types as exceptions (not built-in types))](/errors#Re-exception-types)

* [(E.15: Throw by value, catch exceptions from a hierarchy by reference)](/errors#Re-exception-ref)

* [(E.16: Destructors, deallocation, `swap`, and exception type copy/move construction must never fail)](/errors#Re-never-fail)

* [(E.17: Don't try to catch every exception in every function)](/errors#Re-not-always)

* [(E.18: Minimize the use of explicit `try`/`catch`)](/errors#Re-catch)

* [(E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available)](/errors#Re-finally)

* [(E.25: If you can't throw exceptions, simulate RAII for resource management)](/errors#Re-no-throw-raii)

* [(E.26: If you can't throw exceptions, consider failing fast)](/errors#Re-no-throw-crash)

* [(E.27: If you can't throw exceptions, use error codes systematically)](/errors#Re-no-throw-codes)

* [(E.28: Avoid error handling based on global state (e.g. `errno`))](/errors#Re-no-throw)

* [(E.30: Don't use exception specifications)](/errors#Re-specifications)

* [E.31: Properly order your `catch`-clauses](#Re_catch)

### <a href="#" name="Re-design"></a>E.1: Develop an error-handling strategy early in a design

##### Reason

A consistent and complete strategy for handling errors and resource leaks is hard to retrofit into a system.

### <a href="#" name="Re-throw"></a>E.2: Throw an exception to signal that a function can't perform its assigned task
...
* A constructor that cannot construct an object (failure to establish its class's [(invariant)](/class#Rc-struct))
...
**See also**: [(RAII)](/errors#Re-raii)

**See also**: [(discussion)](/discussion#Sd-noexcept)

##### Note

Before deciding that you cannot afford or don't like exception-based error handling, have a look at the [(alternatives)](/errors#Re-no-throw-raii);
they have their own complexities and problems.
Also, as far as possible, measure before making claims about efficiency.

### <a href="#" name="Re-errors"></a>E.3: Use exceptions for error handling only
...
### <a href="#" name="Re-design-invariants"></a>E.4: Design your error-handling strategy around invariants
...
An [(invariant)](/class#Rc-struct) is a logical condition for the members of an object that a constructor must establish for the public member functions to assume.

##### Enforcement

???

### <a href="#" name="Re-invariant"></a>E.5: Let a constructor establish an invariant, and throw if it cannot
...
**See also**: [(If a constructor cannot construct a valid object, throw an exception)](/class#Rc-throw)

##### Enforcement

Flag classes with `private` state without a constructor (public, protected, or private).

### <a href="#" name="Re-raii"></a>E.6: Use RAII to prevent leaks
...
as a last resort, cleanup actions can be represented by a [(`final_action`)](/errors#Re-finally) object.
...
**See also**: [(Use of `noexcept`)](/errors#Re-noexcept)

##### Enforcement

???

### <a href="#" name="Re-precondition"></a>E.7: State your preconditions

##### Reason

To avoid interface errors.

**See also**: [(precondition rule)](/interfaces#Ri-pre)

### <a href="#" name="Re-postcondition"></a>E.8: State your postconditions

##### Reason

To avoid interface errors.

**See also**: [(postcondition rule)](/interfaces#Ri-post)

### <a href="#" name="Re-noexcept"></a>E.12: Use `noexcept` when exiting a function because of a `throw` is impossible or unacceptable
...
Do not use traditional [(exception-specifications)](/errors#Re-specifications).

##### See also

[(discussion)](/discussion#Sd-noexcept).

### <a href="#" name="Re-never-throw"></a>E.13: Never throw while being the direct owner of an object
...
Sometimes, [(`finally()`)](/errors#Re-finally) can make such unsystematic cleanup a bit more manageable.

### <a href="#" name="Re-exception-types"></a>E.14: Use purpose-designed user-defined types as exceptions (not built-in types)
...
### <a href="#" name="Re-exception-ref"></a>E.15: Throw by value, catch exceptions from a hierarchy by reference
...
Most handlers do not modify their exception and in general we [(recommend use of `const`)](/expr#Res-const).
...
### <a href="#" name="Re-never-fail"></a>E.16: Destructors, deallocation, `swap`, and exception type copy/move construction must never fail
...
* Also, [(make move operations `noexcept`)](/class#Rc-move-noexcept).
...
**See also**: [(discussion)](/discussion#Sd-never-fail)

### <a href="#" name="Re-not-always"></a>E.17: Don't try to catch every exception in every function
...
Let cleanup actions on the unwinding path be handled by [(RAII)](/errors#Re-raii).
...
### <a href="#" name="Re-catch"></a>E.18: Minimize the use of explicit `try`/`catch`
...
* proper resource handles and [(RAII)](/errors#Re-raii)
* [(`finally`)](/errors#Re-finally)

##### Enforcement

??? hard, needs a heuristic

### <a href="#" name="Re-finally"></a>E.19: Use a `final_action` object to express cleanup if no suitable resource handle is available

##### Reason

`finally` from the [(GSL)](/gsl#S-gsl) is less verbose and harder to get wrong than `try`/`catch`.
...
Prefer [(proper resource management objects)](/errors#Re-raii).
Consider `finally` a last resort.

##### Note

Use of `finally` is a systematic and reasonably clean alternative to the old [(`goto exit;` technique)](/errors#Re-no-throw-codes)
...
### <a href="#" name="Re-no-throw-raii"></a>E.25: If you can't throw exceptions, simulate RAII for resource management

##### Reason

Even without exceptions, [(RAII)](/errors#Re-raii) is usually the best and most systematic way of dealing with resources.
...
So, the primary guideline for error handling is "use exceptions and [(RAII)](/errors#Re-raii)."
...
Before condemning exceptions or complaining too much about their cost, consider examples of the use of [(error codes)](/errors#Re-no-throw-codes).
...
### <a href="#" name="Re-no-throw-crash"></a>E.26: If you can't throw exceptions, consider failing fast

##### Reason

If you can't do a good job at recovering, at least you can get out before too much consequential damage is done.

**See also**: [(Simulating RAII)](/errors#Re-no-throw-raii)
...
### <a href="#" name="Re-no-throw-codes"></a>E.27: If you can't throw exceptions, use error codes systematically
...
This approach fits with [(simulated RAII resource management)](/errors#Re-no-throw-raii).
...
`finally` can [(ease the pain a bit)](/errors#Re-finally).
...
We [((prefer exception-based error handling)](/errors#Re-throw) and recommend [keeping functions short)](/functions#Rf-single).

**See also**: [Discussion](#Sd-???)

**See also**: [(Returning multiple values)](/functions#Rf-out-multi)

##### Enforcement

Awkward.

### <a href="#" name="Re-no-throw"></a>E.28: Avoid error handling based on global state (e.g. `errno`)
...
### <a href="#" name="Re-specifications"></a>E.30: Don't use exception specifications
...
For example, see [(Stroustrup94)](/unclassified#Stroustrup94).

##### Note

If no exception can be thrown, use [(`noexcept`)](/errors#Re-noexcept).

##### Enforcement

Flag every exception specification.

### <a href="#" name="Re_catch"></a>E.31: Properly order your `catch`-clauses
