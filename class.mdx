...
* [Prefer to place the interface first in a class](/stdlib#Rl-order)
...
Prefer to place the interface first in a class, [see NL.16](/stdlib#Rl-order).
...
Prefer the order `public` members before `protected` members before `private` members [see](/stdlib#Rl-order).
...
See [discussion](/stdlib#Sd-never-fail).
...
See [this in the Discussion section](/stdlib#Sd-dtor).
...
**See also**: [Discussion](/stdlib#Sd-order)
...
**See also**: [Discussion](/stdlib#Sd-factory)
...
The `swap` implementation technique offers the [strong guarantee](/stdlib#Abrahams01).
...
By writing directly to the target elements, we will get only [the basic guarantee](/stdlib#Abrahams01) rather than the strong guarantee offered by the `swap` technique. Beware of [self-assignment](/class#Rc-copy-self).
...
* This rule is part of the [type-safety profile](/stdlib#Pro-type-downcast).
